...
export function BudgetAssignPanel({ listId, budgetId, messages }: Props) {
  ...
  async function openPicker() {
    setError(null);
    setSelectedId(null);
    const result = await fetchCandidates(listId, budgetId, messages);
    if (!result.ok) {
      setCandidates([]);
  ...
  async function confirmAssign() {
    ...
    setBusy(true);
    setError(null);
    const result = await assignEntry(listId, budgetId, selectedId, messages);
    setBusy(false);
    if (!result.ok) {
      setError(result.error);
      return;
