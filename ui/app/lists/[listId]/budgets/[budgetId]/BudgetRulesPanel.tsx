...
export function BudgetRulesPanel({ listId, budgetId, rules, messages }: Props) {
  ...
  const { pending, error, submit, clearError } = useFormSubmission(
    async (text: string) => createRule(listId, budgetId, text, messages),
    {
      onSuccess: () => {
        setMatchText("");
        router.refresh();
  ...
  async function onDelete(ruleId: string) {
    ...
    setDeleteError(null);
    const result = await deleteRule(listId, budgetId, ruleId, messages);
    setDeletingId(null);
    if (!result.ok) {
