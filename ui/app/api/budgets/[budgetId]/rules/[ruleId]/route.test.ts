...
function context(budgetId: string, ruleId: string) {
  return { params: Promise.resolve({ budgetId, ruleId }) };
}

describe("DELETE /api/budgets/[budgetId]/rules/[ruleId] BFF", () => {
  beforeEach(() => {
    fetchMock.mockReset();
  ...
  it("forwards the cookie and passes through the upstream response", async () => {
    ...
    const request = new Request("http://localhost/api/budgets/b1/rules/r1", {
      method: "DELETE",
      headers: { cookie: "fh_session=tok" },
    });

    const response = await DELETE(request as never, context("b1", "r1") as never);

    expect(response.status).toBe(204);
    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe("http://api.test:8000/budgets/b1/rules/r1");
  ...
  it("passes through a rule-not-found error for a rule on another budget", async () => {
    fetchMock.mockResolvedValue(
      new Response(JSON.stringify({ detail: "Not found.", code: "budget_rule_not_found" }), {
        status: 404,
        headers: { "Content-Type": "application/json" },
      }),
    );
