...
describe("POST /api/lists/{listId}/hide BFF", () => {
  beforeEach(() => {
    fetchMock.mockReset();
  ...
  it("forwards the cookie and passes through a 204 no-content success", async () => {
    ...
    const request = new Request("http://localhost/api/lists/l1/hide", {
      method: "POST",
    ...
    expect(url).toBe("http://api.test:8000/lists/l1/hide");
    expect(init.method).toBe("POST");
    expect((init.headers as Headers).get("Cookie")).toBe("fh_session=tok");
  });

  it("passes through a 403 (owner cannot hide own list) upstream error", async () => {
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({ detail: "Owner cannot hide.", code: "cannot_hide_owned_list" }),
        { status: 403, headers: { "Content-Type": "application/json" } },
      ),
    );

    const request = new Request("http://localhost/api/lists/l1/hide", { method: "POST" });
    const response = await POST(request as never, context("l1"));

    expect(response.status).toBe(403);
    expect((await response.json()).code).toBe("cannot_hide_owned_list");
  });
