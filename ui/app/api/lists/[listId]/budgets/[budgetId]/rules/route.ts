...
/**
 * Same-origin BFF: POST /api/lists/{id}/budgets/{budgetId}/rules
 * → api POST /lists/{id}/budgets/{budgetId}/rules.
 */
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    upstream = await fetch(
      `${getApiInternalUrl()}/lists/${encodeURIComponent(listId)}/budgets/${encodeURIComponent(budgetId)}/rules`,
      {
        method: "POST",
        headers,
