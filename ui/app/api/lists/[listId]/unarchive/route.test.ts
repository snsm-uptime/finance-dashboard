...
describe("POST /api/lists/{listId}/unarchive BFF", () => {
  beforeEach(() => {
    fetchMock.mockReset();
    process.env.API_INTERNAL_URL = "http://api.test:8000";
  });

  it("forwards the cookie and passes through the upstream unarchived list", async () => {
    const body = { id: "l1", name: "Household", owner_id: "u1", is_archived: false };
    ...
    const request = new Request("http://localhost/api/lists/l1/unarchive", {
      method: "POST",
    ...
    expect(url).toBe("http://api.test:8000/lists/l1/unarchive");
  ...
  it("returns 502 bad_gateway when upstream is unavailable", async () => {
    ...
    const request = new Request("http://localhost/api/lists/l1/unarchive", { method: "POST" });
    const response = await POST(request as never, context("l1"));
