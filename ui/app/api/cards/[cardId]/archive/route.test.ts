...
describe("POST /api/cards/{cardId}/archive BFF", () => {
  beforeEach(() => {
    fetchMock.mockReset();
    process.env.API_INTERNAL_URL = "http://api.test:8000";
  });

  it("forwards the cookie and passes through the upstream archived card", async () => {
    const body = { id: "c1", label: "My Visa", iban: "CR05", is_archived: true };
    ...
    const request = new Request("http://localhost/api/cards/c1/archive", {
      method: "POST",
    ...
    expect(url).toBe("http://api.test:8000/cards/c1/archive");
  ...
  it("passes through a 404 upstream error", async () => {
    ...
    const request = new Request("http://localhost/api/cards/c1/archive", { method: "POST" });
    const response = await POST(request as never, context("c1"));
