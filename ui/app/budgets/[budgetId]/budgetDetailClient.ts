/** Client helpers for budget attribution — manual assign, rules, candidates (Story 7.3). */
...
export async function assignEntry(
  ...
  try {
    response = await fetch(`/api/budgets/${encodeURIComponent(budgetId)}/assignments`, {
      method: "POST",
...
export async function unassignEntry(
  ...
  try {
    response = await fetch(
      `/api/budgets/${encodeURIComponent(budgetId)}/assignments/${encodeURIComponent(entryId)}`,
      {
        method: "DELETE",
...
export async function fetchCandidates(
  ...
  let response: Response;
  try {
    response = await fetch(`/api/budgets/${encodeURIComponent(budgetId)}/candidates`, {
      method: "GET",
      headers: { Accept: "application/json" },
      credentials: "same-origin",
    });
...
export async function createRule(
  ...
  let response: Response;
  try {
    response = await fetch(`/api/budgets/${encodeURIComponent(budgetId)}/rules`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Accept: "application/json" },
      credentials: "same-origin",
...
export async function deleteRule(
  ...
  let response: Response;
  try {
    response = await fetch(`/api/budgets/${encodeURIComponent(budgetId)}/rules/${encodeURIComponent(ruleId)}`, {
      method: "DELETE",
      headers: { Accept: "application/json" },
      credentials: "same-origin",
    });
