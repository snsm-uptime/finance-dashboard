...
- [x] **Task 1 — Domain: derive statement cycles from already-fetched ledger entries** (AC: 1, 2, 3, 4)
  - [x] New `api/domain/statement_cycles.py`: pure function `derive_statement_cycles(entries: list[LedgerEntryRecord]) -> list[StatementCycle]`.
...
- [x] **Task 2 — Application: period-filtered reads + available-cycles listing** (AC: 1, 2, 3, 4)
  ...
  - [x] Do not add a `Card.cycle_day`/`cycle_length` field, a `statement_date` column on `import_statements`, or any new migration — Task 1/2 derive everything from data that already exists and is already persisted (`ledger_entries.posted_date`, `ledger_entries.statement_id`, `import_statements.card_id`, `cards.label`).

- [x] **Task 3 — API: query params + cycles endpoint** (AC: 1, 2, 3, 4, 5)
  - [x] `api/api/routes/lists.py`: add optional `period_start: date | None = Query(default=None)`, `period_end: date | None = Query(default=None)` to `GET /{list_id}/expenses` and `GET /{list_id}/balances`; pass through to the application commands from Task 2. Validate `period_start <= period_end` when both given (400 on violation — reuse the existing validation-error response shape used elsewhere in this router).
...
- [x] **Task 4 — UI: cycle selector wired into shared-expenses** (AC: 1, 2, 3, 4, 5)
  ...
  - [x] Card label + date-range as the option label (e.g. `"BAC •••• 2026-06-15 – 2026-07-14"` — exact copy/format is a Dev decision; keep it scannable and free-text-safe since card labels are user-entered, per UX-DR18 "card labels remain free text").
  - [x] Wire the selector into the existing settle-up strip area (above `BalanceStrip`, following the strip → receipts focus order from EXPERIENCE.md's spine table) — do not reuse or extend `TemporalNavigation.tsx` (`ui/app/lists/TemporalNavigation.tsx`, confirmed via research to be an unrelated desktop nav-state toggle for invite/split forms — a naming collision with "temporal" meaning UI chrome state, not calendar time; do not touch it for this story).
  - [x] `asBalances`/`asExpenses` parsing in `page.tsx` needs no change unless Task 3 adds an echoed `period` field to those payloads — if it does, extend the existing defensive-parse pattern (never fabricate values on a parse miss, matching the `balance_status` precedent from Story 5.7).
  - [x] `ui/lib/i18n/lists.ts`: add cycle-selector copy keys (label for the control, per-option format helper text if needed, and the calendar-fallback label e.g. "This month") to both `en` and `es` in `listsMessages`, following the existing key style next to `incompleteDisclosureResolve`.

- [x] **Task 5 — Tests** (AC: all)
  - [x] API integration (Postgres 16, per every prior Epic 4/5 story — not SQLite): extend `test_lists_integration.py` or add a new module:
  ...
  - [x] Domain unit tests for `derive_statement_cycles`/`derive_default_period` (Task 1) — pure function, no DB needed, `Decimal`/date assertions only.
  - [x] UI: `CyclePeriodSelector` — renders nothing for 0 or 1 cycles (AC #3); renders `SoftLedgerSelect` with correct options for 2+; `onChange` produces the correct `?period=` URL. Follow `SettleControls.test.tsx`'s pattern (Story 5.8) for a client-island test, not a full-page RSC render.
  - [x] UI: extend `page.balanceStrip.test.ts`-style pure-helper tests if `asBalances`/`asExpenses` gain any period-echo parsing (only if Task 3's optional response echo is implemented).
  - [x] EN/ES: assert both locale keys exist and render (mirrors the existing i18n test pattern in this codebase — grep for how `incompleteDisclosureResolve` coverage was tested in Story 5.7 and follow it, do not invent a new i18n test style).

### Review Findings

- [x] [Review][Patch] Redundant full-ledger fetches across a single balances request — `GetListBalancesStubService.execute` now fetches `list_ledger_entries` once and passes `entries` through to `resolve_effective_period`, `compute_viewer_balance_crc`, and `compute_viewer_pairwise_edges` [api/application/lists.py]
- [x] [Review][Patch] Partial period query params are silently discarded instead of rejected — route layer now rejects one-sided `period_start`/`period_end` with the same 400 `invalid_period` as a reversed pair [api/api/routes/lists.py]
- [x] [Review][Patch] Duplicate explicit-or-derive-default branching logic in `ListExpensesService.execute` vs `resolve_effective_period` — both now delegate to the shared `resolve_period_bounds` in `domain/statement_cycles.py` [api/application/expenses.py, api/application/lists.py]
- [x] [Review][Patch] Cycle card-label resolution uses only the first-encountered entry's `origin_card_id` per statement, which can be null-first or diverge from other entries in the same statement via the per-row reassign flow — now takes the first non-null `origin_card_id` per statement group; covered by a new regression test [api/application/lists.py, api/tests/test_statement_cycles_integration.py]
- [x] [Review][Patch] No `cyclesLoadError` signal when `/cycles` fetch fails, unlike sibling `splitLoadError`/`membersLoadError` — added, non-blocking per AC #4 [ui/app/lists/[listId]/page.tsx]
- [x] [Review][Patch] `derive_statement_cycles` sort has no tiebreaker for statements sharing the same `period_end` — added `statement_id` as a deterministic secondary sort key [api/domain/statement_cycles.py]
- [x] [Review][Patch] `test_non_member_cycles_access_denied` accepts either 403 or 404 instead of pinning the actual contract — tightened to the sibling routes' documented `404 list_not_found` [api/tests/test_statement_cycles_integration.py]
- [x] [Review][Patch] `test_cycles_empty_falls_back_to_calendar_month` computes its expected window via a live second call rather than freezing/injecting `today` — now captured before the request instead of after [api/tests/test_statement_cycles_integration.py]
- [x] [Review][Patch] Unused `period` echo added to `ListBalancesStubResponse`/`ListExpensesStubResponse` with no consumer — removed [api/api/schemas/lists.py, api/api/routes/lists.py]
- [x] [Review][Patch] `?period=` doesn't defensively coerce a repeated-param `string[]` value from `searchParams` before comparing as a string — coerced to the first value [ui/app/lists/[listId]/page.tsx]
...
- Story 5.8 (PR #93) was already merged to `main` and present in this branch's history (`1a5fc1e Merge pull request #93 ...`) at the start of this session — no rebase needed; implemented directly against the post-5.8 shape of `GetListBalancesStubService`/`page.tsx`.
- API integration/unit/lint verified in-worktree via `docker compose -f docker-compose.yml -f docker-compose.worktree.yml -f docker-compose.test.yml run --rm --build api sh -c "alembic upgrade head && pytest -q"` (843 passed) and `ruff check . && ruff format --check .` (clean).
- UI verified via `npm run typecheck` (clean) and `npx vitest run` (568 passed, 74 files).

### Completion Notes List

- **Task 1**: `api/domain/statement_cycles.py` — pure `derive_statement_cycles`/`derive_default_period`/`current_calendar_month_window`, no FastAPI/SQLAlchemy imports (AD-1). 8 unit tests in `api/tests/test_statement_cycles.py`, all passing.
- **Task 2**: Extended the *actual live* production read paths — `ListExpensesCommand`/`ListExpensesService` (`api/application/expenses.py`, the route-wired service; the story text named `GetListExpensesStubCommand` in `lists.py`, which is dead code only exercised by one legacy test) and `GetListBalancesStubCommand`/`GetListBalancesStubService` (`api/application/lists.py`) — both gained optional `period_start`/`period_end`, defaulting via `derive_default_period` when omitted. Added `conflicts_overlapping_period` to `application/same_price_conflicts.py`, composed with the existing `conflicts_touching_list` (both must pass — AD-10 preserved). New `GetListCyclesService`/`GetListCyclesCommand`/`ListCyclesResult`/`CycleOption` in `lists.py`. Verified (grep of `application/import_session.py`) that `origin_card_id` is set alongside `statement_id` at commit time for every import-sourced row, so it stands in for the `card_id` join without a new query path — confirmed per the story's "verify at implementation time" instruction. Added `SqlAlchemyListRepository.get_card_label(card_id)` (no owner filter, since a shared list's statement can originate from any member's card).
- **Task 3**: `period_start`/`period_end` optional `Query` params on `GET /{list_id}/expenses` and `GET /{list_id}/balances`, 400 `invalid_period` when `period_start > period_end`. New `GET /{list_id}/cycles` route + `ListCyclesResponse`/`CycleItemResponse` schemas, gated by the same `read_balances` ACL action as the balances route. Added an additive `period: {start, end}` echo to both `ListBalancesStubResponse` and `ListExpensesStubResponse` (dev judgment call per the story's explicit invitation to add it) — the UI does not currently consume this echo since it computes the period independently, but it's available for future clients.
- **Task 4**: `ui/app/lists/CyclePeriodSelector.tsx` — client island, renders nothing for 0/1 cycles (AC #3), `SoftLedgerSelect` for 2+, URL-driven `?period=` navigation via `router.push` (no client refetch). `page.tsx` gained `searchParams: Promise<{period?: string}>`, a parallel `/cycles` fetch alongside split/members, and pure exported helpers `asCycles`/`resolveSelectedPeriod` (defensive-parse pattern, never fabricates; a cycles-fetch miss or no-match falls through to no period filtering so the API computes its own default — AC #4 "never blocks settle-up"). New i18n keys `cyclePeriodSelectorLabel`/`cyclePeriodOptionUnknownCard` in both `en`/`es`. Did not touch `TemporalNavigation.tsx` (confirmed unrelated per Dev Notes).
- **Task 5**: New `api/tests/test_statement_cycles_integration.py` (Postgres 16, 7 tests) covering two/one/zero-cycle listings, boundary-inclusive period filtering on `/expenses` and `/balances`, period-scoped incomplete-disclosure (the AC #4 case Story 5.7 deferred here), `period_start > period_end` → 400, and non-member `/cycles` access denial. `CyclePeriodSelector.test.tsx` (6 tests, mirrors `SettleControls.test.tsx`'s client-island pattern) covers the 0/1/2+ cycle render states, option labels (including the unknown-card fallback), `onChange` URL navigation, and an EN/ES copy-key render check. `page.cycles.test.ts` covers `asCycles`/`resolveSelectedPeriod` pure-helper parsing and fallback logic, mirroring `page.balanceStrip.test.ts`'s style.
- Full regression: 843/843 API tests pass, 568/568 UI tests pass, API `ruff check`/`ruff format --check` clean, UI `tsc --noEmit` and `eslint` clean on all touched/new files.

### File List

- `api/domain/statement_cycles.py` (new)
- `api/tests/test_statement_cycles.py` (new)
- `api/tests/test_statement_cycles_integration.py` (new)
...
- `ui/app/lists/CyclePeriodSelector.tsx` (new)
- `ui/app/lists/CyclePeriodSelector.test.tsx` (new)
- `ui/app/lists/[listId]/page.tsx`
- `ui/app/lists/[listId]/page.cycles.test.ts` (new)
...
| 2026-08-31 | Implemented Tasks 1–5: domain cycle derivation, period-filtered expenses/balances + period-aware incomplete-disclosure + new `/cycles` endpoint, `CyclePeriodSelector` UI wired into shared-expenses, and full test coverage (domain unit, Postgres integration, UI component/pure-helper). Full regression green (843 API tests, 568 UI tests). Status set to `review`. |
| 2026-08-31 | Code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor): 10 patch findings applied — deterministic cycle-sort tiebreak, fixed a real card-label bug (null-first/diverged `origin_card_id` per statement, with regression test), rejected one-sided period query params, deduplicated period-resolution logic via shared `resolve_period_bounds`, consolidated a triple ledger fetch in `GetListBalancesStubService`, added a `cyclesLoadError` signal, coerced a repeated-param edge case in `?period=`, removed a dead response-echo field, and tightened two loose tests. Full regression re-verified green (844 API tests, 568 UI tests; ruff/tsc/eslint clean). Status set to `done`. |
