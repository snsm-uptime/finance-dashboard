...
- [x] [Review][Patch] No gate on card `routing_mode` for Bulk-committed statements — AC #1 assumes only review-routed cards reach Bulk, but neither `ImportStatementModel` nor the Bulk-commit path carries or checks a card/`routing_mode` association at all (that linkage doesn't exist yet — Stories 4.4/4.6's territory). Resolved per user decision: added an inline comment on `AssignBulkImportService.execute` plus a canary unit test (`test_staged_statement_record_has_no_unchecked_card_routing_field`) that fails loud if `card_id`/`routing_mode` ever lands on a statement record without an accompanying routing_mode gate. [api/application/import_session.py:277-283, api/tests/test_import_session_application.py]
- [x] [Review][Patch] Bulk-imported rows bypass `validate_manual_expense`'s money/data-integrity invariants — `AssignBulkImportService.execute` builds `ManualExpenseDraft` directly from parsed `CanonicalLine` fields with no amount-bounds/quantization or description-length check (currency has partial defense via `MaterializeFxService`'s BCCR-support check, but nothing else does); both draft origins write to the same `ledger_entries` table Story 3.2 established these invariants for. Fixed: added `domain.import_session.validate_bulk_candidate_row` (magnitude/2dp/description-length only — unlike `validate_manual_expense`, it accepts negative/zero amounts since real `payment`/`credit_note` statement lines are legitimately non-positive), wired into `AssignBulkImportService.execute`, mapped `InvalidCanonicalLineError` to 422 in the bulk-commit route. 7 new unit tests + 1 application test. [api/domain/import_session.py, api/application/import_session.py:301-310, api/api/routes/import_sessions.py]
- [x] [Review][Patch] Concurrent double bulk-commit (or a list deleted mid-request) raises an unhandled `IntegrityError` instead of a clean 409/404 — two simultaneous bulk-commit calls can both pass `validate_bulk_commit_eligible` before either persists; only `uq_import_batches_statement_id`/list FK backstops it at flush time, and the route's exception handling doesn't catch SQLAlchemy `IntegrityError`. Fixed: `commit_statement_batch` now wraps the batch-row insert in `begin_nested()` + `except IntegrityError → ImportSessionAlreadyCommittedError` (same pattern as `cards.py`/`repositories.py`). New Postgres integration test calls the repo directly to simulate the race and asserts the clean domain error. [api/adapters/persistence/import_sessions.py:169-186, api/tests/test_import_sessions_integration.py]
- [x] [Review][Patch] `import_batch_id` traceability (this story's central AD-4 guarantee — a ledger row traces back to its committing batch) is never asserted by any test and isn't exposed by `ExpenseItemResponse` either; the happy-path integration test checks `ledger_entry_count` and `payer_id`/`provenance` but not the FK itself. Fixed: happy-path integration test now queries `LedgerEntryModel` directly via `db_session` and asserts every row's `import_batch_id` matches the returned batch id. [api/tests/test_import_sessions_integration.py:242-274]
- [x] [Review][Patch] Missing `None` guard on the re-fetched `statement_row` in `commit_statement_batch` — `self._session.get(ImportStatementModel, statement_id)` result is dereferenced (`.status = ...`) with no check, so a bad id would raise a bare `AttributeError` instead of a domain error. Fixed: raises `ImportSessionNotFoundError` when `None`. [api/adapters/persistence/import_sessions.py:212-215]
- [x] [Review][Defer] AC #2's `?listId=` pre-select mechanism is unreachable dead code — the only real entry point (`UploadPanel.tsx`'s "Assign to a list" link) never constructs the query param; self-documented in Task 4.1 as awaiting a future list-scoped upload entry point. [ui/app/upload/UploadPanel.tsx:133-138, ui/app/upload/bulk/[sessionId]/BulkReviewPanel.tsx:51-54] — deferred, pre-existing scope gap acknowledged in the story's own dev notes
- [x] [Review][Defer] AC #3's "payer remains editable" has no backing edit endpoint anywhere in the app — but this matches Story 3.2 hand-expense payer semantics exactly (no payer-edit route exists for any expense today), so it's a system-wide gap, not a regression unique to this story. [api/api/routes/lists.py (only PATCH .../origin exists)] — deferred, pre-existing across both hand and imported expenses
- [x] [Review][Defer] A staged statement with zero candidate rows produces a valid but empty Import Batch — `validate_bulk_commit_eligible` only checks that *some* statement is staged, not that each has rows. Benign (no ledger corruption) but untested. [api/domain/import_session.py:37-52] — deferred, low-likelihood edge case
- [x] [Review][Defer] `_session_record` eagerly builds full `CanonicalLine` objects (incl. date/Decimal conversion) for every candidate row on every call site — `create_session`/`get_session`/`discard_session` don't need `candidate_rows`, only `AssignBulkImportService` does. [api/adapters/persistence/import_sessions.py:32-52] — deferred, minor overhead, no functional impact
- [x] [Review][Defer] FX materialization in Bulk commit is sequential per candidate row with no batching and no NFR timing test (Story 4.6 added one for upload; this story adds none for commit) — inherited from the existing Story 3.5 manual-expense pattern, not a new regression. [api/application/import_session.py:296-317] — deferred, pattern reused as designed
- [x] [Review][Defer] Application-tier unit tests use a no-op 1:1 `_FakeFxService` for every currency, so non-CRC `amount_crc`/`fx_rate`/`fx_fallback` flow through `commit_statement_batch` is only ever exercised at the Postgres-integration tier. [api/tests/test_import_session_application.py:313-321] — deferred, test-layering gap only
- [x] [Review][Defer] `_own_list_id` integration-test helper blindly takes `lists[0]` with no assertion that exactly one list exists — brittle if default-list registration behavior ever changes. [api/tests/test_import_sessions_integration.py] — deferred, test scaffolding nit
