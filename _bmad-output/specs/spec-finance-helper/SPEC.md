...
- **CAP-11**
  - **intent:** List members open a shared-expenses view for a statement/billing cycle showing, for the signed-in member, You are owed / You owe / Balance in CRC, with a receipt-style item list, optional Simplify group plan, plain-text copy, and Settle = payables already paid (clean for the actor).
  - **success:** Settle-up is CRC-only using materialized FX at commit (BCCR purchase/statement date; nearest prior + fallback flag if missing; no user FX override); foreign lines show original + CRC for audit; pairwise columns are viewer-centric; Simplify reduces transfers without writing a bank payment ledger or saying “paid”; Copy uses existing CopyButton; after Settle the viewer’s You owe is clean and You are owed remains for later reminders; only included line types (`line-types.md`) affect allocations; incomplete/quarantined periods are disclosed; when cycles differ the user selects which cycle to view.
