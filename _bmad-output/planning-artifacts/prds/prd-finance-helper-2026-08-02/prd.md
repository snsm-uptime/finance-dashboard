...
**Users are peers.** Every user authenticates with their own Google account, gets a personal list on signup, uploads their own statements, creates as many lists as they want, and shares any of them with other users. There is no privileged product role. What a user can see is determined entirely by which lists they belong to.
...
The user may also dismiss an entire file outright, distinct from skipping an individual statement within it.

`[OPEN]` What the user can do from the comparison view. Dismissing is available; whether they can also accept a partial parse decides whether the product is strict-with-diagnostics or strict-with-human-override.

`[OPEN]` Whether the comparison view is failure-only or available for any statement as a verification aid.
...
This is a live tension with the chosen front end rather than a settled requirement. Streamlit reruns its script on every interaction and offers no native gesture handling, no first-class PDF viewer, and a weak responsive story — and v1's review flow now needs all three. The user-facing requirements are recorded here; reconciling them with the framework belongs to architecture.
...
- Google authentication, with users, personal lists created on signup, user-created lists, and list membership
...
| Question | Origin |
|---|---|
| Whether the split ratio lives on the list or varies within it | Lists |
...
| Whether the comparison view allows accepting a partial parse, or only dismissing | Statement ingestion |
| Whether the comparison view is failure-only or always available | Statement ingestion |
| Whether Streamlit can deliver gestures, PDF rendering, and mobile layout | Form factor; architecture |
