...
- **FR-54:** A list owner can **archive** a list they own, and a card owner can **archive** a registered card. Archived lists/cards are hidden from their default views (Lists homepage, Cards panel) and excluded from create/registration-form interactions in that view; a toggle (box icon) on each surface switches to an archived-only view, resetting to non-archived on navigation away — same interaction pattern as FR-51. Archiving preserves all list/card data, membership, and history and is reversible (unarchive). Archiving a card does not affect its historical import batches or ledger lines; archiving a list does not remove membership, but non-owners cannot archive it. *(New 2026-09-04 — see ui/todo.md item 4, Epic 9.)*
- **FR-55:** On the parse-failure comparison surface (FR-25), the user can hand-enter the failed statement's transaction as a manual expense (FR-21) inline, without leaving the surface. Desktop shows a third column (extracted items | original PDF | manual-entry form) alongside the existing two; phone swaps the extracted-items region for the form (PDF stays fixed in the lower half, unaffected — FR-25). The form is visible in full immediately, pre-filled from whatever the failed parse's evidence already extracted (description/amount/currency/date, per row); it is not gated behind picking a list first — payer/split fields populate once a list is chosen. Submitting creates the expense and dismisses the statement (FR-26) in one action. A back/close control returns to the prior view (extracted items on phone, two-column on desktop) without dismissing anything. *(New 2026-09-23 — see Sprint Change Proposal 2026-09-23.)*
